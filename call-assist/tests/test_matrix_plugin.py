...
class TestMatrixPluginIntegration:
    ...
    async def test_matrix_call_flow(
        ...
        creds_response = await broker_server.update_credentials(
            creds_request, timeout=10.0
        )
        ...
        call_request = CallRequest(
            ...
            preferred_capabilities=MediaCapabilities(
                video_codecs=["H264"], audio_codecs=["OPUS"], webrtc_support=True
            ),
        )
        ...
        async with MatrixTestClient() as matrix_client:
            ...
            assert (
                call_message_found
            ), "No call message or Matrix call event found in room"

        # Terminate the call
        term_request = CallTerminateRequest(call_id=call_id)
    ...
    async def test_matrix_call_with_invalid_room(
        ...
        if call_response.success:
            ...
            term_request = CallTerminateRequest(call_id=call_response.call_id)
