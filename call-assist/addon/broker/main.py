...
class CallAssistBroker(BrokerIntegrationBase, CallPluginBase):
    ...
    async def update_configuration(
        ...
        try:
            logger.info(
                f"Updating configuration: {len(configuration_request.camera_entities)} cameras, {len(configuration_request.media_player_entities)} media players"
            )
    ...
    async def update_credentials(
        self, credentials_request: CredentialsRequest
    ) -> CredentialsResponse:
        ...
        try:
            ...
            logger.info(
                f"Updating credentials for account: {credentials_request.account_id} ({credentials_request.protocol})"
            )
            logger.info(
                f"Received credentials: {list(credentials_request.credentials.keys())}"
            )

            # Create or update account in database
            existing_account = get_account_by_protocol_and_id(
                credentials_request.protocol, credentials_request.account_id
            )
            ...
            if success:
                return CredentialsResponse(
                    success=True,
                    message=f"Credentials updated for {credentials_request.account_id} ({credentials_request.protocol})",
                )
            else:
                ...
                return CredentialsResponse(
                    success=False,
                    message=f"Plugin initialization failed for {credentials_request.account_id} ({credentials_request.protocol})",
                )

        except Exception as e:
            logger.error(
                f"Credentials update failed for {credentials_request.account_id} ({credentials_request.protocol}): {e}"
            )
    ...
    async def initiate_call(self, call_request: CallRequest) -> CallResponse:
        ...
        try:
            ...
            logger.info(
                f"Initiating call {call_id}: {call_request.protocol} to {call_request.target_address}"
            )
            ...
            log_call_start(
                call_id,
                call_request.protocol,
                call_request.account_id,
                call_request.target_address,
                call_request.camera_entity_id,
    ...
    async def terminate_call(
        self, call_terminate_request: CallTerminateRequest
    ) -> CallTerminateResponse:
        ...
        try:
            ...
            if call_id not in self.active_calls:
                return CallTerminateResponse(
                    success=False, message=f"Call {call_id} not found"
                )
            ...
            log_call_end(
                call_id, "TERMINATED", {"reason": "User requested termination"}
            )
            ...
            return CallTerminateResponse(
                success=True, message=f"Call {call_id} terminated"
            )
    ...
    async def stream_call_events(
        self, betterproto_lib_pydantic_google_protobuf_empty
    ...
    async def stream_contact_updates(
        self, betterproto_lib_pydantic_google_protobuf_empty
    ) -> AsyncIterator[ContactUpdate]:
        ...
        try:
            ...
            for contact in self.contacts.values():
                contact_update = ContactUpdate(
                    type=ContactUpdateType.CONTACT_UPDATE_INITIAL_LIST, contact=contact
                )
    ...
    async def stream_health_status(
        ...
        try:
            ...
            health = HealthStatus(
                healthy=True, component="broker", message="Broker running normally"
            )
            yield health
        ...
        try:
            ...
            if self.configuration:
                for (
                    camera_entity_id,
                    camera_name,
                ) in self.configuration.camera_entities.items():
                    for (
                        media_player_id,
                        media_player_name,
                    ) in self.configuration.media_player_entities.items():
                        station_id = (
                            f"station_{camera_entity_id}_{media_player_id}".replace(
                                ".", "_"
                            )
                        )
                        ...
                        for call_id, call_info in self.active_calls.items():
                            if (
                                call_info.camera_entity_id == camera_entity_id
                                and call_info.media_player_entity_id == media_player_id
                            ):
                                active_call = call_info
                                current_state = self._call_state_to_entity_state(
                                    call_info.state
                                )
                                break
                        ...
                        station_entity = EntityDefinition(
                            ...
                            attributes={
                                ...
                                "protocols": ",".join(
                                    self.configuration.enabled_protocols
                                ),
                                "current_call_id": (
                                    active_call.call_id if active_call else ""
                                ),
                            },
            ...
            for protocol in self.plugin_manager.get_available_protocols():
                ...
                status_entity = EntityDefinition(
                    ...
                    state=self._plugin_state_to_entity_state(
                        plugin_state, is_configured
                    ),
                    attributes={
                        "protocol": protocol,
            ...
            broker_entity = EntityDefinition(
                ...
                attributes={
                    ...
                    "configured_cameras": (
                        str(len(self.configuration.camera_entities))
                        if self.configuration
                        else "0"
                    ),
                    "configured_players": (
                        str(len(self.configuration.media_player_entities))
                        if self.configuration
                        else "0"
                    ),
                    "available_protocols": ",".join(
                        self.plugin_manager.get_available_protocols()
                    ),
    ...
    def _plugin_state_to_entity_state(
        self, plugin_state: Optional[PluginState], is_configured: bool
    ) -> str:
        ...
        try:
            ...
            for protocol in available_protocols:
                ...
                if caps:
                    ...
                    resolutions = [
                        Resolution(
                            width=res.width, height=res.height, framerate=res.framerate
                        )
                    ...
                    for account_creds in configured_accounts:
                        plugin_caps.append(
                            PluginCapabilities(
                                ...
                                available=(
                                    account_creds.is_valid
                                    and plugin_state is not None
                                    and plugin_state != PluginState.ERROR
                                ),
            ...
            return SystemCapabilities(
                broker_capabilities=broker_caps, available_plugins=plugin_caps
            )
    ...
    def _generate_protocol_schema(self, protocol: str, metadata) -> ProtocolSchema:
        ...
        schema_def = schema_definitions.get(
            protocol,
            {
    ...
    def get_account_configuration(
        ...
        if account and account.is_valid:
            plugin_instance = self.plugin_manager.get_plugin_instance(
                protocol, account_id
            )
    ...
    def is_account_configured(self, protocol: str, account_id: str) -> bool:
        ...
        return (
            ...
            and self.get_account_configuration(protocol, account_id) is not None
        )
    ...
    async def _notify_plugin_credentials(
        self, account_creds: AccountCredentials
    ) -> bool:
        ...
        if success:
            logger.info(
                f"Plugin instance initialized for {account_creds.account_id} ({account_creds.protocol})"
            )
            # Update the credentials validity if successful
            account_creds.is_valid = True
        else:
            logger.error(
                f"Failed to initialize plugin instance for {account_creds.account_id} ({account_creds.protocol})"
            )
    ...
    async def _forward_call_to_plugin(self, call_info: CallInfo):
        ...
        response = await self.plugin_manager.start_call_on_account(
            call_info.protocol, call_info.account_id, call_request
        )
        if response and response.success:
            # Update call state
            call_info.state = response.state
            logger.info(
                f"Call {call_info.call_id} forwarded to {call_info.protocol} plugin (account: {call_info.account_id})"
            )
        else:
            call_info.state = CallState.FAILED
            logger.error(
                f"Failed to forward call {call_info.call_id} to {call_info.protocol} plugin (account: {call_info.account_id})"
            )
    ...
    async def _terminate_call_on_plugin(self, call_info: CallInfo):
        ...
        call_request = CallEndRequest(
            call_id=call_info.call_id, reason="User requested termination"
        )
        ...
        if response and response.success:
            logger.info(
                f"Call {call_info.call_id} terminated on {call_info.protocol} plugin (account: {call_info.account_id})"
            )
        else:
            logger.error(
                f"Failed to terminate call {call_info.call_id} on {call_info.protocol} plugin (account: {call_info.account_id})"
            )
    ...
    def on_plugin_health_changed(self, protocol: str, healthy: bool, message: str):
        ...
        logger.info(
            f"Plugin {protocol} health: {'healthy' if healthy else 'unhealthy'} - {message}"
        )
...
async def serve(
    ...
    try:
        ...
        logger.info(
            f"Available plugins: {broker.plugin_manager.get_available_protocols()}"
        )
