...
class FormField:
    ...
    def validate(self) -> bool:
        ...
        if self.config.get('required', False) and not self.value:
            self.validation_errors.append(f"{self.config.get('display_name', self.key)} is required")
            return False
...
class FormGenerator:
    ...
    def generate_form(self, schema: Dict[str, Any], container: Optional[ui.column] = None) -> Dict[str, FormField]:
